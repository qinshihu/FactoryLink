...
![トレイアイコン](industrial-gateway\images\1 (2).png)

3. ブラウザが自動的に設定ページを開きます（デフォルト `http://localhost:8000`）

![設定ページ](industrial-gateway\images\1 (1).png)

4. PLCのIP、ポイントテーブル、MQTTアドレスを設定します

  ![デバイス設定](industrial-gateway\images\1 (4).png)
...
- **Excelインポート**：Excelからポイント設定を一括インポート
...
| **デバイス設定** | デバイスの追加/編集/削除、プロトコル別設定項目、ポイントテーブルCRUD、Excelインポート/テンプレートダウンロード、接続テスト |
| **システム設定** | MQTT設定、収集間隔、再接続戦略、起動時自動実行、ログ表示（レベルフィルタリング対応） |
...
![WeChat QRコード](industrial-gateway\images\公众号背面.png)
